...
def main():
  ...
  file.write('  <Axes>\n')
  file.write('    <XAxis min="{}" max="{}">List Size</XAxis>\n'.format(xmin, xmax))
  file.write('    <YAxis min="{}" max="{}">Microseconds</YAxis>\n'.format(min(yList), 60))
  file.write('  </Axes>\n')
  
  file.write('  <Sequence title="Average Access Time vs List Size" color="red">\n')

  for i in range(len(xList)):
    file.write('    <DataPoint x="{}" y="{}"/>\n'.format(xList[i], yList[i]))
  ...
  for i in range(100):
    ...
    endtime = datetime.datetime.now()
    deltaT = endtime - starttime
    yList[index] = yList[index] + deltaT.total_seconds() * 1000

  file.write('  <Sequence title="Access Time Distribution" color="blue">\n')

  for i in range(len(xList)):
    if xList[i] > 0:
      file.write('    <DataPoint x="{}" y="{}"/>\n'.format(i, yList[i]/xList[i]))
